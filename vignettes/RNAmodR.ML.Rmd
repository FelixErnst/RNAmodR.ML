...
object using the `setMLModel` function.
...
visualizeDataByCoord(sequenceData(me),mod[["4"]][1])
```
...
machine learning models.
 ...
 aggregate the sequence data as is, provide a custom `cbind` implementation
 for the `aggregateData` function. Since you probably don't want to hold to
 copies of the same data in memory (one as the aggregate, one as the sequence 
 data), you can create a `Modifier` object, with the additional argument
 `dont.save.seqdata`. For more details have a look at `?Modifier`.
 3. in a deep learning context, if a coordinate is selected without a flanking 
 value, a 2D tensor is returned (sample, values). This can be converted into a 
 3D tensor by providing a flanking value.
